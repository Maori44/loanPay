...
export class ProfileUserViewComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    console.log(this.userLoggedInAccountId);
    this.init();
    this.routerPath = this.commonService.getRouterPath();
    
    if(this.OrganizationType=="Sponsor" || this.OrganizationType=="CRO"){
      this.isOrganization=false;
    }
    else if(this.OrganizationType=="Organization"){
  ...
  getUserData() {
    let obj = {
      guid: this.manageUserList,
      orgType:this.OrganizationType
    }
    this.commonService.GetUserDetails(obj).subscribe(res => {
      if (res) {
        if (res.statusCode === HttpStatusCode.StatusCode200) {
  ...
  onSubmit(type: string) {
    ...
    if (type == 'save') {     
      ...
      //  otherName: this.Form.value.otherName,
        ...
        userID: this.userId,
        userGuid: this.userLoggedInAccountId,
        createdByLoginID: 5,
        isEdit: false,
    ...
    this.commonService.UpdateUserDetails(obj).subscribe(res => {
      if (res) {
        if (res.statusCode === HttpStatusCode.StatusCode200) {
